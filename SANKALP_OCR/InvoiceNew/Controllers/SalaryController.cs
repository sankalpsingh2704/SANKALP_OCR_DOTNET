...
namespace InvoiceNew.Controllers
{
	public class SalaryController : Controller
	{
		...
		public string AddtoOverTimeXml(DataTable model, string LocationId, string Month, string Year, string TallyComment)
		{
			...
			{
				string ledgerName = "";
				if (dr["Category"].ToString().ToLower().Contains("body shop"))
				{
		...
		public string AddtoSalaryXml(DataTable model, string LocationId, string Month, string Year, string TallyComment)
		{
            ...
            Dictionary<String, decimal> Ledgerlist = new Dictionary<string, decimal>();
			foreach (DataRow dr in model.Rows)
			{
                if (dr["NetAmount"] != null && Decimal.Parse(dr["NetAmount"].ToString()) >0 )
                {
                    NetAmount = Decimal.Parse(dr["NetAmount"].ToString());
                    LedgerModel lst = lstModel.Where(a => a.LedgerName.Contains("Salary Payable") && a.LedgerName.ToLower().Contains(locationCode.ToLower()) && !a.LedgerName.Contains("RSA") && !a.LedgerName.Contains("Warranty")).FirstOrDefault();

                    if (lst != null)
                    {
                        GrandTotal += Decimal.Parse(dr["NetAmount"].ToString());
                        Ledgerlist.Add(lst.LedgerName, NetAmount);
                    }
                }
                if (dr["PT"] != null && Decimal.Parse(dr["PT"].ToString()) > 0)
                {
                    PT = Decimal.Parse(dr["PT"].ToString());
                   
                    LedgerModel lst = lstModel.Where(a => a.LedgerName.Contains("Professional Tax Payable") && a.LedgerName.ToLower().Contains(locationCode.ToLower()) && !a.LedgerName.Contains("RSA") && !a.LedgerName.Contains("Warranty")).FirstOrDefault();

                    if (lst != null)
                    {
                        GrandTotal += Decimal.Parse(dr["PT"].ToString());
                        Ledgerlist.Add(lst.LedgerName, Decimal.Parse(dr["PT"].ToString()));
                    }
                }
                if (dr["PFAmount"] != null && Decimal.Parse(dr["PFAmount"].ToString()) >0)
                {
                    PFAmount = Decimal.Parse(dr["PFAmount"].ToString());
                
                    LedgerModel lst = lstModel.Where(a => a.LedgerName.Contains("Provident Fund Payable") && a.LedgerName.ToLower().Contains(locationCode.ToLower())  && !a.LedgerName.Contains("RSA") && !a.LedgerName.Contains("Warranty")).FirstOrDefault();

                    if (lst != null)
                    {
                        GrandTotal += Decimal.Parse(dr["PFAmount"].ToString());
                        Ledgerlist.Add(lst.LedgerName, Decimal.Parse(dr["PFAmount"].ToString()));
                    }
                }
                if (dr["MedicalAL"] != null && Decimal.Parse(dr["MedicalAL"].ToString()) >0 )
                {
                    MedicalAL = Decimal.Parse(dr["MedicalAL"].ToString());
                  
                    LedgerModel lst = lstModel.Where(a => a.LedgerName.Contains("Mediclaim - Deduction") &&  !a.LedgerName.Contains("RSA") && !a.LedgerName.Contains("Warranty")).FirstOrDefault();

                    if (lst != null)
                    {
                        GrandTotal += Decimal.Parse(dr["MedicalAL"].ToString());
                        Ledgerlist.Add(lst.LedgerName,Decimal.Parse(dr["MedicalAL"].ToString()));
                    }
                }
                if (dr["TDS"] != null && Decimal.Parse(dr["TDS"].ToString()) >0 )
                {
                    TDS = Decimal.Parse(dr["TDS"].ToString());
 
                    LedgerModel lst = lstModel.Where(a => a.LedgerName.Contains("Tds - Salary")).FirstOrDefault();

                    if (lst != null)
                    {
                        GrandTotal += Decimal.Parse(dr["TDS"].ToString());
                        Ledgerlist.Add(lst.LedgerName, Decimal.Parse(dr["TDS"].ToString()));
                    }
                }
			...
			xdoc.Load(Server.MapPath("~/tallyxml/SalaryVoucherFinal.xml"));
			//XmlNode Cname = xdoc.SelectSingleNode("ENVELOPE/BODY/IMPORTDATA/REQUESTDESC/STATICVARIABLES/SVCURRENTCOMPANY");
			//Cname.InnerText = branchname;
			//XmlNode Rname = xdoc.SelectSingleNode("ENVELOPE/BODY/IMPORTDATA/REQUESTDATA/TALLYMESSAGE/COMPANY/REMOTECMPINFO.LIST/REMOTECMPNAME");
			//Rname.InnerText = branchname;
...
"<ISPARTYLEDGER>No</ISPARTYLEDGER>" +
"<ISLASTDEEMEDPOSITIVE>Yes</ISLASTDEEMEDPOSITIVE>" +
"<AMOUNT>-"+GrandTotal.ToString()+"</AMOUNT>" +
"<VATEXPAMOUNT>-" + GrandTotal.ToString() + "</VATEXPAMOUNT>" +
"<SERVICETAXDETAILS.LIST></SERVICETAXDETAILS.LIST>" +
"<CATEGORYALLOCATIONS.LIST>" +
"<CATEGORY>Service</CATEGORY>" +
"<ISDEEMEDPOSITIVE>Yes</ISDEEMEDPOSITIVE>" +
"<COSTCENTREALLOCATIONS.LIST>" +
"<NAME>"+CultureInfo.CurrentCulture.TextInfo.ToTitleCase(LocList[2])+"</NAME>" +
"<AMOUNT>-" + GrandTotal.ToString() + "</AMOUNT>" +
"</COSTCENTREALLOCATIONS.LIST>" +
"</CATEGORYALLOCATIONS.LIST>" +
"<BANKALLOCATIONS.LIST></BANKALLOCATIONS.LIST>" +
...
"</ALLLEDGERENTRIES.LIST>";
 parentNode.InsertAfter(xfrag, parentNode.LastChild);
    ...
    "<OLDAUDITENTRYIDS>-1</OLDAUDITENTRYIDS>" +
    "</OLDAUDITENTRYIDS.LIST>" +
    "<LEDGERNAME>"+ledger.Key.ToString() +"</LEDGERNAME>" +
    "<GSTCLASS/>" +
    "<ISDEEMEDPOSITIVE>No</ISDEEMEDPOSITIVE>" +
    "<LEDGERFROMITEM>No</LEDGERFROMITEM>" +
    "<REMOVEZEROENTRIES>No</REMOVEZEROENTRIES>" +
    "<ISPARTYLEDGER>Yes</ISPARTYLEDGER>" +
    "<ISLASTDEEMEDPOSITIVE>No</ISLASTDEEMEDPOSITIVE>" +
    "<AMOUNT>"+ ledger.Value.ToString() +"</AMOUNT>" +
    "<VATEXPAMOUNT>"+ ledger.Value.ToString() +"</VATEXPAMOUNT>" +
    "<SERVICETAXDETAILS.LIST></SERVICETAXDETAILS.LIST>" +
    "<CATEGORYALLOCATIONS.LIST>" +
    "<CATEGORY>Service</CATEGORY>" +
    "<ISDEEMEDPOSITIVE>No</ISDEEMEDPOSITIVE>" +
    ...
    "</ALLLEDGERENTRIES.LIST>";
		...
		public FileResult ExportExcel(string Model)
		{
			...
			using (ExcelPackage pck = new ExcelPackage())
			{
				...
				foreach (var col in model.SearchList)
				{
					if (col == "PresentDays" || col == "PayDays" || col == "Basic" || col == "HRA" || col == "LTA" || col == "MedicalAL" || col == "Education" || col == "TotalEarning" || col == "PFAmount" || col == "ESI" || col == "PT" || col == "TDS" || col == "Others" || col == "Advance" || col == "TotalDeduction" || col == "NetAmount")
				...
				//ws.Cells["AJ2:AJ"+dt.Rows.Count].Style.Numberformat.Format = "##0.0";
				
				//ws.Cells["AJ2:AJ" + dt.Rows.Count+1].Style.Numberformat.Format = "0";
				//ws.Cells["AJ2:AJ" + dt.Rows.Count].Style.Numberformat
		...
		public FileResult ExportOTExcel(string Model)
		{
			...
			using (ExcelPackage pck = new ExcelPackage())
			{
				...
				foreach (var col in model.SearchList)
				{
					if (col == "ESI"|| col == "OTAmount" || col == "NetAmount" || col == "DoubleHours" || col == "TotalHours" || col == "PerHourSalary" || col == "PerDaySalary" || col == "Basic" || col == "GrossSalary")
					{
						dt.Columns.Add(col,typeof(decimal));
					}
					else
		...
		{
			PaymentInputModel pim = new PaymentInputModel();
			pim.PageTypes = new List<SelectListItem>{ new SelectListItem {Text = "Salary",Value = "Salary" }, new SelectListItem { Text = "OverTime", Value = "OverTime" }, new SelectListItem { Text = "Incentive", Value = "Incentive" } };
			pim.Month = DateTime.Now.Month;
			pim.Year = DateTime.Now.Year;
			return View(pim);
		}
		[HttpPost]
		public JsonResult PaymentUpload(PaymentInputModel pim)
		{

			List<PaymentListModel> paylist = new List<PaymentListModel>();
			var Payed = false;
			con.Open();
			SqlCommand cmd = new SqlCommand("getPayment", con);
			cmd.Parameters.AddWithValue("@LocationName",pim.PageName);
			cmd.Parameters.AddWithValue("@Month", pim.Month);
			cmd.Parameters.AddWithValue("@Year", pim.Year);
			cmd.CommandType = CommandType.StoredProcedure;
			using (SqlDataReader sdr = cmd.ExecuteReader())
			{
				while (sdr.Read())
				{
					Payed = bool.Parse(sdr["PaymentUpdated"].ToString());
				}
				while (sdr.NextResult())
				{
					while (sdr.Read())
					{
						PaymentListModel plm = new PaymentListModel();
						plm.Amount = Decimal.Parse(sdr["Amount"].ToString());
						plm.BankName = sdr["BankName"].ToString();
						paylist.Add(plm);
					}
				}
			}
			con.Close();
			return Json(new { paylist,Payed});
		}
		[HttpPost]
		public JsonResult SendPayment(PaymentInputModel pim)
		{
			List<SendPaymentModel> paylist = new List<SendPaymentModel>();
			try
			{		  
				con.Open();
				SqlCommand cmd = new SqlCommand("getTallyPayment", con);
				cmd.Parameters.AddWithValue("@LocationName", pim.PageName);
				cmd.Parameters.AddWithValue("@Month", pim.Month);
				cmd.Parameters.AddWithValue("@Year", pim.Year);
				cmd.CommandType = CommandType.StoredProcedure;
				...
				while (sdr.Read())
				{
					SendPaymentModel plm = new SendPaymentModel();
					plm.Amount = decimal.Parse(sdr["Amount"].ToString());
					//plm.BankName = sdr["BankName"].ToString();
					plm.Code = sdr["Code"].ToString();
					plm.Service	= sdr["Service"].ToString();
				...
				if (pim.PageName == "Salary")
				{
					AddtoSalaryPayment(pim, paylist);
				}
				else if (pim.PageName == "Incentive")
				{
					AddtoIncentivePayment(pim, paylist);
				}
				else if (pim.PageName == "OverTime")
				{
					AddtoOverTimePayment(pim, paylist);
				}
			}
			catch(Exception ex)
			{
				return Json(new { paylist = ex.Message });
			}
            return Json(new { paylist });
			
		}

        public string AddtoIncentivePayment(PaymentInputModel pm, List<SendPaymentModel> paylist)
        {
            ...
            xdoc.Load(Server.MapPath("~/tallyxml/IncentivePaymentVoucher.xml"));
            XmlNode root = xdoc.DocumentElement;
            XmlNode parentNode = xdoc.SelectSingleNode("ENVELOPE/BODY/IMPORTDATA/REQUESTDATA/TALLYMESSAGE/VOUCHER");
            ...
            //EFFECTIVEDATENode.InnerText = DateTime.Now.ToString("yyyymmdd");
            XmlNode NARRATIONNode = xdoc.SelectSingleNode("ENVELOPE/BODY/IMPORTDATA/REQUESTDATA/TALLYMESSAGE/VOUCHER/NARRATION");
            NARRATIONNode.InnerText = pm.TallyComment;
            ...
            VOUCHERNUMBERNode.InnerText = "20180201";


            string monthname = CultureInfo.CurrentCulture.DateTimeFormat.GetAbbreviatedMonthName(pm.Month) + " " + pm.Year.ToString().Substring(2, 2);


            XmlNodeList ALLLEDGERENTRIESLIST = xdoc.SelectNodes("ENVELOPE/BODY/IMPORTDATA/REQUESTDATA/TALLYMESSAGE/VOUCHER/ALLLEDGERENTRIES.LIST");

            Dictionary<string, decimal> banknameslistTotal = new Dictionary<string, decimal>();
            ...
            decimal GrandTotal = 0;
                ...
                foreach (var item in paylist)
                {
                    ...
                    LedgerModel lst = lstModel.Where(a => a.LedgerName.Contains("Incentive Payable") && a.LedgerName.ToLower().Contains(item.Code.ToLower())).FirstOrDefault();
                ...
                banklistTotal.Add("KOTAK MAHENDRA BANK", GrandTotal);
                ...
                foreach (var mod in paylist)
                {

                    LedgerModel lst = lstModel.Where(a => a.LedgerName.Contains("Incentive Payable") && a.LedgerName.ToLower().Contains(mod.Code.ToLower())).FirstOrDefault();
...
"<OLDAUDITENTRYIDS>-1</OLDAUDITENTRYIDS>" +
"</OLDAUDITENTRYIDS.LIST>" +
"<LEDGERNAME>" + lst.LedgerName + "</LEDGERNAME>" +
"<GSTCLASS/>" +
"<ISDEEMEDPOSITIVE>Yes</ISDEEMEDPOSITIVE>" +
"<LEDGERFROMITEM>No</LEDGERFROMITEM>" +
"<REMOVEZEROENTRIES>No</REMOVEZEROENTRIES>" +
...
"<VATEXPAMOUNT>-" + mod.Amount + "</VATEXPAMOUNT>" +
"<SERVICETAXDETAILS.LIST></SERVICETAXDETAILS.LIST>" +
"<CATEGORYALLOCATIONS.LIST>" +
"<CATEGORY>Service</CATEGORY>" +
"<ISDEEMEDPOSITIVE>Yes</ISDEEMEDPOSITIVE>" +
...
foreach (var item  in banklistTotal)
            { 
...
"<VATEXPAMOUNT>" + item.Value + "</VATEXPAMOUNT>" +
"<SERVICETAXDETAILS.LIST></SERVICETAXDETAILS.LIST>" +
"<CATEGORYALLOCATIONS.LIST>" +
"<CATEGORY>Service</CATEGORY>" +
"<ISDEEMEDPOSITIVE>No</ISDEEMEDPOSITIVE>" +
...
"<SECONDARYSTATUS/>" +
"<BANKPARTYNAME>Incentive Payable(Amb)</BANKPARTYNAME>" +
...
"</ALLLEDGERENTRIES.LIST>";
                ...
                using (StreamReader sr = new StreamReader(objResponse.GetResponseStream()))
                {
                    ...
                    {
						con.Open();
						SqlCommand com = new SqlCommand("UpdateTallyPayIncStatus", con);
						com.CommandType = CommandType.StoredProcedure;
        ...
        public string AddtoSalaryPayment(PaymentInputModel pm, List<SendPaymentModel>  paylist)
        {
            ...
            xdoc.Load(Server.MapPath("~/tallyxml/SalaryPaymentVoucher.xml"));
            XmlNode root = xdoc.DocumentElement;
            XmlNode parentNode = xdoc.SelectSingleNode("ENVELOPE/BODY/IMPORTDATA/REQUESTDATA/TALLYMESSAGE/VOUCHER");
           ...
           decimal GrandTotal = 0;
                ...
                foreach (var item in paylist)
                {
                    ...
                    LedgerModel lst = lstModel.Where(a => a.LedgerName.Contains("Salary Payable") && a.LedgerName.ToLower().Contains(item.Code.ToLower())).FirstOrDefault();
                ...
                banklistTotal.Add("Kotak Mahindra Bank", GrandTotal);
                ...
                foreach (var mod in paylist)
                {
                    ...
                    LedgerModel lst = lstModel.Where(a => a.LedgerName.Contains("Salary Payable") && a.LedgerName.ToLower().Contains(mod.Code.ToLower())).FirstOrDefault();
                    ...
                    if (lst != null)
                    {
                        ...
                        "<OLDAUDITENTRYIDS>-1</OLDAUDITENTRYIDS>" +
                        "</OLDAUDITENTRYIDS.LIST>" +
                        "<LEDGERNAME>Salary Payable-" + mod.Code + "</LEDGERNAME>" +
                        "<GSTCLASS/>" +
                        "<ISDEEMEDPOSITIVE>Yes</ISDEEMEDPOSITIVE>" +
                        "<LEDGERFROMITEM>No</LEDGERFROMITEM>" +
                        "<REMOVEZEROENTRIES>No</REMOVEZEROENTRIES>" +
                        ...
                        "<BANKALLOCATIONS.LIST></BANKALLOCATIONS.LIST>" +
                        "<BILLALLOCATIONS.LIST>" +
                        "<NAME>"+ monthname +"- " + mod.Code + "</NAME>" +
                        "<BILLTYPE>Agst Ref</BILLTYPE>" +
                        "<TDSDEDUCTEEISSPECIALRATE>No</TDSDEDUCTEEISSPECIALRATE>" +
                        "<AMOUNT>-" + mod.Amount + "</AMOUNT>" +
                        "<INTERESTCOLLECTION.LIST></INTERESTCOLLECTION.LIST>" +
                        "<STBILLCATEGORIES.LIST></STBILLCATEGORIES.LIST>" +
                        "</BILLALLOCATIONS.LIST><INTERESTCOLLECTION.LIST>" +
            ...
            foreach (var item in banklistTotal)
            {
                    ...
                    "<NAME></NAME>" +
                    "<TRANSACTIONTYPE></TRANSACTIONTYPE>" +
                    "<PAYMENTFAVOURING>Salary Payable-Amb</PAYMENTFAVOURING>" +
                    "<CHEQUECROSSCOMMENT></CHEQUECROSSCOMMENT>" +
                    "<INSTRUMENTNUMBER></INSTRUMENTNUMBER>" +
                    "<UNIQUEREFERENCENUMBER></UNIQUEREFERENCENUMBER>" +
                    "<STATUS>No</STATUS>" +
                    "<PAYMENTMODE></PAYMENTMODE>" +
                    "<BANKPARTYNAME>Salary Payable-Amb</BANKPARTYNAME>" +
                    "<ISCONNECTEDPAYMENT>No</ISCONNECTEDPAYMENT>" +
                    "<ISSPLIT>No</ISSPLIT>" +
                    "<ISCONTRACTUSED>No</ISCONTRACTUSED>" +
                    "<ISACCEPTEDWITHWARNING>No</ISACCEPTEDWITHWARNING>" +
        ...
        public string AddtoOverTimePayment(PaymentInputModel pm, List<SendPaymentModel> paylist)
        {
            ...
            xdoc.Load(Server.MapPath("~/tallyxml/OverTimePaymentVoucher.xml"));
            XmlNode root = xdoc.DocumentElement;
            XmlNode parentNode = xdoc.SelectSingleNode("ENVELOPE/BODY/IMPORTDATA/REQUESTDATA/TALLYMESSAGE/VOUCHER");
            ...
            decimal GrandTotal = 0;
                ...
                foreach (var item in paylist )
                {
                    ...
                    LedgerModel lst = lstModel.Where(a => a.LedgerName.Contains("Overtime Payable") && a.LedgerName.ToLower().Contains(item.Code.ToLower())).FirstOrDefault();
                ...
                foreach (var mod in paylist)
                {
                    ...
                    LedgerModel lst = lstModel.Where(a => a.LedgerName.Contains("Overtime Payable") && a.LedgerName.ToLower().Contains(mod.Code.ToLower())).FirstOrDefault();
               ...
               foreach(var item in banklistTotal)
                {
                        ...
                        "<NAME></NAME>" +
                        "<TRANSACTIONTYPE></TRANSACTIONTYPE>" +
                        "<PAYMENTFAVOURING>Overtime Payable-Amb</PAYMENTFAVOURING>" +
                        "<CHEQUECROSSCOMMENT></CHEQUECROSSCOMMENT>" +
                        "<INSTRUMENTNUMBER></INSTRUMENTNUMBER>" +
                        "<UNIQUEREFERENCENUMBER></UNIQUEREFERENCENUMBER>" +
                        "<STATUS>No</STATUS>" +
                        "<PAYMENTMODE></PAYMENTMODE>" +
                        "<BANKPARTYNAME>Overtime Payable-Amb</BANKPARTYNAME>" +
                        "<ISCONNECTEDPAYMENT>No</ISCONNECTEDPAYMENT>" +
                        "<ISSPLIT>No</ISSPLIT>" +
                        "<ISCONTRACTUSED>No</ISCONTRACTUSED>" +
                        "<ISACCEPTEDWITHWARNING>No</ISACCEPTEDWITHWARNING>" +
                ...
                using (StreamReader sr = new StreamReader(objResponse.GetResponseStream()))
                {
                    ...
                    {
						con.Open();
						SqlCommand com = new SqlCommand("UpdateTallyPayOTStatus", con);
						com.CommandType = CommandType.StoredProcedure;
