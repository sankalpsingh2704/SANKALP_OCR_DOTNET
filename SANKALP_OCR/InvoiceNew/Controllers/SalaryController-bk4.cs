 ...
 parentNode.InsertAfter(xfrag, parentNode.LastChild);
    ...
    "</ALLLEDGERENTRIES.LIST>";
        ...
        public string AddtoIncentivePayment(PaymentInputModel pm, List<SendPaymentModel> paylist)
        {
            ...
            var banknameslistTotal = paylist.GroupBy(a => a.BankName).Select(a => new { Bank = a.Key, }).ToList();

            Dictionary<string, decimal> banklistTotal = new Dictionary<string, decimal>();
            decimal GrandTotal = 0;
            foreach (var jmm in banknameslistTotal.Where(a=>a.Bank== "KOTAK MAHENDRA BANK"))
            {
                foreach (var item in paylist.Where(a => a.BankName == jmm.Bank))
                {
                ...
                banklistTotal.Add(jmm.Bank, GrandTotal);
            }
            ...
            foreach (var item in banklistTotal)
            {
                ...
                var banklist = paylist.Where(sm => sm.BankName == item.Key).ToList();

                foreach (var mod in banklist)
                {
...
"</ALLLEDGERENTRIES.LIST>";
                ...
                if (item.Key == "KOTAK MAHENDRA BANK")
                {
